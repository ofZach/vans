...
guiBaseObject::guiBaseObject(){
	...
	if( !initialColorsLoaded ){
		...
		gOutlineColor.setColor(70, 70, 70, 255);
		gOutlineColor.setSelectedColor(70, 70, 70, 255);

		gBgColor.setColor(30, 30, 30, 80);
		gBgColor.setSelectedColor(30, 30, 30, 80);
...
void guiBaseObject::keyReleased(int k){
	for(unsigned int i = 0; i < children.size(); i++){
		children[i]->keyReleased(k);
	}
}
...
string guiBaseObject::getVarsAsString(){
	...
	for(int i = 0; i < value.getNumValues(); i++){
		...
		}else if( dataType == SG_TYPE_INT ) str += ofToString(value.getValueI(i), 0);
		else if( dataType == SG_TYPE_BOOL ) str += ofToString(value.getValueB(i), 0);
	}
