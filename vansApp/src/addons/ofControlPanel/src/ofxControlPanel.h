...
class ofxControlPanel: public guiBaseObject{
	...
	public:
		...
		guiTypeSlider * addSlider(string sliderName, float value , float min, float max, bool isInt = false);

        guiTypeSlider * addSlider(string sliderName, string xmlName, float value , float min, float max, bool isInt = false);
        guiType2DSlider * addSlider2D(string sliderName, float valueX, float valueY, float minX, float maxX, float minY, float maxY, bool isInt = false);
...
//		guiTypeTextInput* addTextInput( string name, string text, int width );
		...
		//setters and getters
		string getCurrentPanelName(); //Legacy. Deprecated. 
		...
		void showStatusBar(bool bShowBar);
