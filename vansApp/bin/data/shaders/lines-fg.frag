...
float doMap(float inputValue, float inMin, float inMax, float outMin, float outMax, bool bClamp){
		
		float outVal = ((inputValue - inMin) / (inMax - inMin) * (outMax - outMin) + outMin);
...
float rand(vec2 co, float scale){
    return fract(sin(dot(co.xy ,vec2(12.9898,78.233))) * scale);
}
